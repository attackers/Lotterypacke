...
@property(nonatomic,retain)NSArray<NSString *>*rightContentList;
...
+(void)showFanDianListWithOnView:(UIView *)superView
                           title:(NSString *)title
                 LeftContentList:(NSArray<NSString *> *)leftContentList
                rightContentList:(NSArray<NSString *> *)rightContentList
{
    CPSelectedOptionsAgoView *optionsView = [CPSelectedOptionsAgoView createViewFromNib];
    optionsView.frame = CGRectMake(0, 0, superView.width, superView.height);
    optionsView.rightContentList = rightContentList;
    optionsView.options = leftContentList;
    optionsView.title = title;
    [optionsView showOnView:superView];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_rightContentList.count>0) {
        ...
        cell.detailTextLabel.text = _rightContentList[indexPath.row];
        return cell;
