...
- (void)viewDidLoad {
    ...
    NSString *imageUrl = [[CPGlobalDataManager shareGlobalData].domainUrlString wayStringByAppendingPathComponent:[_rechargeInfo DWStringForKey:@"img"]];
    [_qrCodeImageView sd_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:nil options:SDWebImageRetryFailed completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        
        if (image) {
            CGFloat imgViewWidth = image.size.width * _qrCodeImageView.height/image.size.height;
            CGFloat finalImgViewWidth = imgViewWidth>=self.view.width*0.8?self.view.width*0.8:imgViewWidth;
            
            _qrCodeImageView.frame = CGRectMake((self.view.width-finalImgViewWidth)/2.0f, _qrCodeImageView.originY, finalImgViewWidth, finalImgViewWidth *_qrCodeImageView.height/imgViewWidth);
        }
